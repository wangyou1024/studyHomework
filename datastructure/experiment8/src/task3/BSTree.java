...
public class BSTree {
    ...
    public Student search(Comparable number,BiTreeNode biTreeNode){
        if (biTreeNode == null){
            return null;
        }
        if (number.compareTo(((RecordNode)biTreeNode.getData()).getKey()) == 0){
            return ((Student)((RecordNode)biTreeNode.getData()).getElement());
        }
        else if (number.compareTo(((RecordNode)biTreeNode.getData()).getKey()) > 0){
            return search(number,biTreeNode.getRchild());
        }
        else
            return search(number,biTreeNode.getLchild());
    }
